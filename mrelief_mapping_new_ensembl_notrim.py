...
MRELIEF_in = open(sys.argv[1],"r")
alignment_mapping_in = open(sys.argv[2],"r")
file_out = open(sys.argv[5],"w")



# Mapping from trimmed alignment to full alignment; incorporate in MRELIEF data frame

MRELIEF = pd.read_table(MRELIEF_in,header=None)

print MRELIEF

mapping = pd.read_csv(alignment_mapping_in,sep=" ",header=None)

MRELIEF.insert(0,'trim_map', mapping.iloc[:,1])
...
column_one = mapping.iloc[:,0]
column_one = list(column_one)
column_one = pd.DataFrame(column_one)
...
MRELIEF.insert(0,'aln_pos',column_one)

MRELIEF.insert(2,'aln_map_'+sys.argv[4],column_three)

MRELIEF.to_csv(sys.argv[5],sep="\t")
